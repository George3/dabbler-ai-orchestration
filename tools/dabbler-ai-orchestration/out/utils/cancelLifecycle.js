...
exports.wasRestored = wasRestored;
exports.readCancellationState = readCancellationState;
exports.cancelSessionSet = cancelSessionSet;
exports.restoreSessionSet = restoreSessionSet;
 ...
 */
function wasRestored(sessionSetDir) {
    return (fs.existsSync(path.join(sessionSetDir, RESTORED_FILENAME)) &&
...
function atomicWriteFile(filePath, content) {
    ...
    const tmpPath = path.join(directory, `.${base}.${process.pid}-${Math.random().toString(36).slice(2, 8)}.tmp`);
    try {
...
function toV4OnDiskShape(state, sessionSetDir) {
    ...
    let normalized;
    try {
        normalized = (0, progress_1.normalizeToV4Shape)(state, specMdPath);
    }
    catch (exc) {
    ...
    const out = {
        ...
        sessionSetName: normalized.sessionSetName ??
    ...
    const isPlanless = !inputHasSessions &&
...
async function restoreSessionSet(sessionSetDir, reason = "") {
    ...
    if (!fs.existsSync(cancelledPath)) {
        throw new Error(`restoreSessionSet: ${cancelledPath} does not exist; nothing to restore`);
    }
...
//# sourceMappingURL=cancelLifecycle.js.map