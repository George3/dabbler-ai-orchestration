...
exports.synthesizeNotStartedState = synthesizeNotStartedState;
exports.ensureSessionStateFile = ensureSessionStateFile;
...
// verificationVerdict.
const SCHEMA_VERSION = progress_1.SCHEMA_VERSION_V4;
...
function readTotalSessionsFromSpec(sessionSetDir) {
    ...
    const titles = (0, progress_1.extractSessionTitlesFromSpec)(specPath);
    if (titles.length === 0)
        return null;
 ...
 * not-started.
 */
function synthesizeNotStartedState(sessionSetDir) {
...
function loadCanonicalStatus(filePath) {
    ...
    if (typeof parsed !== "object" || parsed === null) {
        throw new Error(`${filePath}: session-state.json must contain a JSON object`);
    }
    const status = parsed.status;
    if (typeof status !== "string") {
        throw new Error(`${filePath}: session-state.json missing string 'status' field`);
    }
