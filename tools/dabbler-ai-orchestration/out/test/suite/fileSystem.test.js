...
suite("fileSystem — isMidSetComplete (Set 023 Session 4)", () => {
    ...
    function writeEvents(setDir, sessionsClosed) {
        const events = sessionsClosed
    ...
    test("F1: completedSessions includes currentSession + no events ledger → not mid-set", () => {
        ...
        assert.strictEqual((0, fileSystem_1.isMidSetComplete)(statePath), false);
        fs.rmSync(dir, { recursive: true });
    });
    ...
    test("F2: array missing currentSession + ledger missing closeout → mid-set", () => {
        ...
        assert.strictEqual((0, fileSystem_1.isMidSetComplete)(statePath), true);
        fs.rmSync(dir, { recursive: true });
    });
    ...
    test("F5: non-array completedSessions falls through to ledger check", () => {
        ...
        assert.strictEqual((0, fileSystem_1.isMidSetComplete)(statePath2), false);
        fs.rmSync(dir, { recursive: true });
    });
    ...
    test("observability warn does not fire on non-override paths", () => {
        ...
        try {
            ...
            assert.strictEqual((0, fileSystem_1.isMidSetComplete)(aPath), false);
            ...
            writeEvents(b, [1, 2, 3]);
            assert.strictEqual((0, fileSystem_1.isMidSetComplete)(bPath), false);
            ...
            writeEvents(c, [1, 2]);
            assert.strictEqual((0, fileSystem_1.isMidSetComplete)(cPath), true);
        }
        finally {
            console.warn = originalWarn;
        }
