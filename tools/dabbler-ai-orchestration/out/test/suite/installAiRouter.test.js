...
function recordingSpawner(responses) {
    const calls = [];
    let i = 0;
    const spawner = async (cmd, args, opts) => {
        const call = { cmd, args: [...args], cwd: opts?.cwd };
        calls.push(call);
        const partial = typeof responses === "function" ? responses(call) : responses[i++] ?? { exitCode: 0, stdout: "", stderr: "" };
...
function seedExistingVenv(workspaceRoot, name = ".venv") {
...
function gitHubSpawner(opts) {
    const lsRemote = opts.lsRemoteOutput ??
        [
...
suite("aiRouterInstall — isAiRouterNotInstalled detector", () => {
    test("matches the precise stderr line python -m emits", () => {
        const stderr = "/usr/bin/python: Error while finding module specification for 'ai_router.queue_status' (ModuleNotFoundError: No module named 'ai_router')";
        assert.strictEqual((0, aiRouterInstall_1.isAiRouterNotInstalled)(stderr), true);
    });
    ...
    test("returns false for unrelated import errors", () => {
        ...
        assert.strictEqual((0, aiRouterInstall_1.isAiRouterNotInstalled)(stderr), false);
    });
    ...
    test("returns false for empty stderr", () => {
        assert.strictEqual((0, aiRouterInstall_1.isAiRouterNotInstalled)(""), false);
    });
});
...
suite("aiRouterInstall — deriveVenvFromPythonPath", () => {
    test("returns the venv root for a Windows venv interpreter path", () => {
        const root = (0, aiRouterInstall_1.deriveVenvFromPythonPath)("C:\\proj\\.venv\\Scripts\\python.exe");
    ...
    test("returns the venv root for a POSIX venv interpreter path", () => {
        const root = (0, aiRouterInstall_1.deriveVenvFromPythonPath)("/proj/.venv/bin/python");
        assert.strictEqual(root, "/proj/.venv");
    });
    test("returns null for a bare command name", () => {
        assert.strictEqual((0, aiRouterInstall_1.deriveVenvFromPythonPath)("python"), null);
        assert.strictEqual((0, aiRouterInstall_1.deriveVenvFromPythonPath)("python3"), null);
    });
    test("returns null when the parent dir is not Scripts/ or bin/", () => {
        assert.strictEqual((0, aiRouterInstall_1.deriveVenvFromPythonPath)("/usr/local/bin-other/python"), null);
    });
});
...
suite("aiRouterInstall — resolveLatestReleaseTag", () => {
    ...
    test("picks the highest semver tag and ignores pre-release suffixes", async () => {
        ...
        const tag = await (0, aiRouterInstall_1.resolveLatestReleaseTag)(deps(spawner));
...
suite("aiRouterInstall — PyPI install (happy path)", () => {
    test("installs from PyPI in an existing venv and writes the install-method marker", async () => {
        ...
        const outcome = await (0, aiRouterInstall_1.installAiRouter)({
            workspaceRoot: ws,
            pythonPath: "python",
            spawner,
            fileOps: realFileOps(),
        ...
        assert.strictEqual(calls[0].cmd, (0, aiRouterInstall_1.venvPython)(venv));
        assert.deepStrictEqual(calls[0].args, ["-m", "pip", "install", aiRouterInstall_1.PYPI_PACKAGE_NAME]);
        // Marker file written
        const marker = path.join(ws, aiRouterInstall_1.INSTALL_METHOD_REL);
    ...
    test("offers to create .venv when no venv is detected and uses it on accept", async () => {
        ...
        assert.deepStrictEqual(calls[1].args, ["-m", "pip", "install", aiRouterInstall_1.PYPI_PACKAGE_NAME]);
        fs.rmSync(ws, { recursive: true, force: true });
    });
    ...
    test("update mode passes -U and reads the install-method marker as the default source", async () => {
        ...
        const outcome = await (0, aiRouterInstall_1.updateAiRouter)({
            workspaceRoot: ws,
            pythonPath: "python",
            spawner,
            fileOps: realFileOps(),
        ...
        assert.deepStrictEqual(calls[0].args, ["-m", "pip", "install", "-U", aiRouterInstall_1.PYPI_PACKAGE_NAME]);
        fs.rmSync(ws, { recursive: true, force: true });
    });
    test("seeds workspace ai_router/router-config.yaml from the installed package on a fresh PyPI install", async () => {
        ...
        assert.strictEqual(outcome.ok, true, outcome.message);
        assert.strictEqual(outcome.routerConfigPreserved, true, "expected the materialized config to set routerConfigPreserved=true");
        const workspaceConfig = path.join(ws, aiRouterInstall_1.ROUTER_CONFIG_REL);
    ...
    test("PyPI install leaves an existing workspace router-config.yaml alone (operator-tuned values survive)", async () => {
        ...
        const { spawner, calls } = recordingSpawner((call) => {
            if (call.args[0] === "-m" && call.args[1] === "pip")
                return { exitCode: 0 };
        ...
        assert.strictEqual(outcome.ok, true, outcome.message);
        assert.strictEqual(outcome.routerConfigPreserved, false, "no materialization should occur when the file already exists");
    ...
    test("uses the venv derived from the configured pythonPath (with pyvenv.cfg marker) instead of hunting for .venv/", async () => {
        ...
        assert.strictEqual(outcome.ok, true, outcome.message);
        assert.strictEqual(outcome.venvPath, customVenv, "expected the install command to use the venv that owns the configured pythonPath");
        // pip was invoked via that venv's python, not via the workspace `.venv/`.
        assert.strictEqual(calls[0].cmd, (0, aiRouterInstall_1.venvPython)(customVenv));
        fs.rmSync(ws, { recursive: true, force: true });
    });
    test("creating a missing .venv when configured pythonPath points inside that nonexistent venv uses bare 'python' as bootstrap (no ENOENT)", async () => {
        ...
        const ws = makeTmpWorkspace();
        const venvPyShape = process.platform === "win32"
        ...
        assert.strictEqual(outcome.ok, true, outcome.message);
        assert.strictEqual(bootstrapCmd, "python", "expected the bootstrap to fall back to bare 'python' rather than the nonexistent venv interpreter");
        fs.rmSync(ws, { recursive: true, force: true });
    });
    test("creating .venv with a real existing absolute interpreter (e.g. /usr/bin/python3 shape) honors that interpreter — no overcorrection to bare 'python'", async () => {
        ...
        assert.strictEqual(outcome.ok, true, outcome.message);
        assert.strictEqual(bootstrapCmd, fakeUsrBinPython, "expected an existing system interpreter to be used as-is, not overridden by bare 'python'");
        fs.rmSync(ws, { recursive: true, force: true });
    });
    test("absolute system interpreter (e.g. /usr/bin/python3 shape) is NOT misidentified as a venv — falls through to workspace detection", async () => {
        ...
        assert.strictEqual(outcome.ok, true, outcome.message);
        assert.strictEqual(outcome.venvPath, workspaceVenv, "expected fall-through to workspace .venv/ when the configured python path is not actually inside a venv");
        assert.strictEqual(calls[0].cmd, (0, aiRouterInstall_1.venvPython)(workspaceVenv));
        fs.rmSync(ws, { recursive: true, force: true });
    });
});
...
suite("aiRouterInstall — GitHub install (happy path)", () => {
    test("resolves the latest released tag, sparse-clones, copies into a persistent location, and editable-installs that path", async () => {
        ...
        assert.strictEqual(outcome.resolvedRef, "v0.1.0", "expected the latest released tag to be resolved (v0.1.0 in the stub ls-remote payload)");
        ...
        assert.strictEqual(calls[1].args[branchIdx + 1], "v0.1.0", "clone must check out the resolved latest tag, not the default branch");
        assert.strictEqual(calls[2].cmd, "git");
        assert.deepStrictEqual(calls[2].args.slice(-3), ["set", "ai_router", "pyproject.toml"]);
        assert.strictEqual(calls[3].cmd, (0, aiRouterInstall_1.venvPython)(venv));
        // Editable install must point at the persistent .dabbler/ai-router-src/, NOT a tmpdir.
        const stableSrc = path.join(ws, aiRouterInstall_1.GITHUB_CHECKOUT_REL);
        ...
        assert.strictEqual(fs.readFileSync(path.join(ws, aiRouterInstall_1.INSTALL_METHOD_REL), "utf8").trim(), "github");
        fs.rmSync(ws, { recursive: true, force: true });
    });
    test("preserves an existing router-config.yaml across the sparse-checkout copy", async () => {
        const ws = makeTmpWorkspace();
        seedExistingVenv(ws);
        const routerConfig = path.join(ws, aiRouterInstall_1.ROUTER_CONFIG_REL);
        ...
        const { spawner } = gitHubSpawner({
            populateClone: (tmp) => {
                fs.mkdirSync(path.join(tmp, "ai_router"), { recursive: true });
                fs.writeFileSync(path.join(tmp, "ai_router", "router-config.yaml"), "# UPSTREAM DEFAULT\n");
            },
        });
        const outcome = await (0, aiRouterInstall_1.installAiRouter)({
            workspaceRoot: ws,
    ...
    test("removes a stale workspace ai_router/ before copying the new sparse checkout (no ghost files)", async () => {
        ...
        assert.strictEqual(outcome.ok, true, outcome.message);
        assert.ok(!fs.existsSync(ghost), "stale file from previous install must be wiped by the upgrade");
        assert.ok(fs.existsSync(path.join(ws, "ai_router", "__init__.py")));
        fs.rmSync(ws, { recursive: true, force: true });
    });
    test("threads a configured repoUrl through both ls-remote and clone (fork support)", async () => {
        ...
        assert.ok(lsRemoteCall.args.includes(fork), "ls-remote should query the configured fork URL, not the upstream default");
        const cloneCall = calls.find((c) => c.cmd === "git" && c.args[0] === "clone");
        assert.ok(cloneCall.args.includes(fork), "git clone should target the configured fork URL");
        fs.rmSync(ws, { recursive: true, force: true });
    });
    test("router-config.yaml survives a copyDir failure that occurs AFTER removing dstAiRouter (writeFile must mkdir parent)", async () => {
        ...
        assert.ok(fs.existsSync(routerConfig), "operator-tuned router-config.yaml must survive even when copy fails after dstAiRouter is wiped");
    ...
    test("install does NOT report success when stash restore fails after a successful copy (data-loss safeguard)", async () => {
        ...
        assert.strictEqual(outcome.ok, false, "install must NOT report success when the operator's config could not be restored");
    ...
    test("router-config.yaml is restored when the editable install fails", async () => {
        ...
        assert.strictEqual(outcome.routerConfigPreserved, true, "expected routerConfigPreserved=true even on install-step failure");
        fs.rmSync(ws, { recursive: true, force: true });
    });
});
// ---------- aborts ----------
...
suite("aiRouterInstall — install-method marker round-trip", () => {
    test("malformed marker is ignored (defaults back to PyPI)", async () => {
        ...
        assert.strictEqual(presentedDefault, "pypi", "expected unknown marker contents to fall through to the PyPI default");
        fs.rmSync(ws, { recursive: true, force: true });
    });
});
...
suite("ProviderQueuesProvider — graceful not-installed (parseFetchResult)", () => {
    test("returns reason=module_not_installed for the ai_router import error", () => {
        const r = (0, ProviderQueuesProvider_1.parseFetchResult)(fakeRun({
            exitCode: 1,
            stderr: "Error while finding module specification for 'ai_router.queue_status' (ModuleNotFoundError: No module named 'ai_router')",
        }));
    ...
    test("leaves reason undefined for unrelated non-zero exits", () => {
        const r = (0, ProviderQueuesProvider_1.parseFetchResult)(fakeRun({ exitCode: 2, stderr: "RuntimeError: queue corrupt" }));
        assert.strictEqual(r.ok, false);
        if (!r.ok) {
...
suite("ProviderQueuesProvider — failure invalidates cache (no stale-data masking)", () => {
    test("a successful fetch followed by a module_not_installed failure renders notInstalled, not the cached payload", async () => {
        ...
        assert.ok(first.length > 0 && first[0].kind === "provider", "first refresh should surface the cached success payload");
        nowMs += 10000; // advance past CACHE_TTL_MS so the next call refetches
        const second = await provider.getChildren();
        assert.strictEqual(second.length, 1);
        assert.strictEqual(second[0].kind, "notInstalled", "second refresh must surface notInstalled, not the cached success payload");
    });
    test("a successful fetch followed by an unrelated non-zero failure renders the red-error info node, not the cached payload", async () => {
        ...
        assert.strictEqual(second[0].isError, true);
    });
});
...
suite("ProviderQueuesProvider — graceful not-installed tree-item rendering", () => {
    function makeNotInstalledProvider() {
        return new ProviderQueuesProvider_1.ProviderQueuesProvider({
            getWorkspaceRoot: () => "/ws",
            fetchPayload: async () => ({
                ok: false,
                message: "ai_router is not installed in the configured Python environment.",
    ...
    test("module_not_installed surfaces a notInstalled root with a clickable child", async () => {
        ...
        const actionItem = (0, ProviderQueuesProvider_1.buildTreeItem)(children[0]);
    ...
    test("notInstalled root uses a neutral info icon (not the red error icon) and a distinct contextValue", async () => {
        const provider = makeNotInstalledProvider();
        const top = await provider.getChildren();
        const rootItem = (0, ProviderQueuesProvider_1.buildTreeItem)(top[0]);
...
suite("ProviderHeartbeatsProvider — graceful not-installed (parseFetchResult)", () => {
    test("returns reason=module_not_installed for the ai_router import error", () => {
        const r = (0, ProviderHeartbeatsProvider_1.parseFetchResult)(fakeRun({
            exitCode: 1,
            stderr: "Error while finding module specification for 'ai_router.heartbeat_status' (ModuleNotFoundError: No module named 'ai_router')",
        }), 60);
        assert.strictEqual(r.ok, false);
        if (!r.ok) {
            assert.strictEqual(r.reason, "module_not_installed");
    ...
    test("leaves reason undefined for unrelated non-zero exits", () => {
        const r = (0, ProviderHeartbeatsProvider_1.parseFetchResult)(fakeRun({ exitCode: 2, stderr: "ConnectionRefusedError: signal file busy" }), 60);
        assert.strictEqual(r.ok, false);
        if (!r.ok) {
            assert.strictEqual(r.reason, undefined);
            assert.match(r.message, /exited 2/);
...
suite("ProviderHeartbeatsProvider — graceful not-installed tree-item rendering", () => {
    ...
    test("module_not_installed surfaces a notInstalled root with a clickable child", async () => {
        ...
        assert.strictEqual(children.length, 1);
        assert.strictEqual(children[0].kind, "notInstalledAction");
        const actionItem = (0, ProviderHeartbeatsProvider_1.buildTreeItem)(children[0]);
        assert.strictEqual(actionItem.command?.command, "dabblerSessionSets.installAiRouter");
    ...
    test("notInstalled root uses a distinct contextValue from the red-error path", async () => {
        const provider = makeNotInstalledProvider();
        const top = await provider.getChildren();
        const rootItem = (0, ProviderHeartbeatsProvider_1.buildTreeItem)(top[0]);
...
//# sourceMappingURL=installAiRouter.test.js.map