...
function v3State(sessions, overrides = {}) {
    return {
...
function v4State(sessions, overrides = {}) {
    return {
...
function sess(number, status, extra = {}) {
    return {
        number,
...
suite("normalizeToV4Shape — v3 input", () => {
    test("v3 input gets schemaVersion 4", () => {
        const out = (0, progress_1.normalizeToV4Shape)(v3State([sess(1, progress_1.SESSION_STATUS_NOT_STARTED)], { status: "not-started" }), MISSING_PATH);
    ...
    test("v3 promotes top orchestrator to in-progress session", () => {
        const out = (0, progress_1.normalizeToV4Shape)(v3State([
        ...
        ], { orchestrator: ORCH, startedAt: "2026-05-26T10:00:00-04:00" }), MISSING_PATH);
        const ip = out.sessions.find((s) => s.status === "in-progress");
    ...
    test("v3 promotes top completedAt to last completed session", () => {
        const out = (0, progress_1.normalizeToV4Shape)(v3State([
            ...
            sess(3, progress_1.SESSION_STATUS_IN_PROGRESS),
        ], {
            completedAt: "2026-05-26T12:00:00-04:00",
            verificationVerdict: "VERIFIED",
        }), MISSING_PATH);
        const lastComplete = out.sessions.filter((s) => s.status === "complete").pop();
    ...
    test("v3 between-sessions orchestrator goes to last completed", () => {
        const out = (0, progress_1.normalizeToV4Shape)(v3State([sess(1, progress_1.SESSION_STATUS_COMPLETE), sess(2, progress_1.SESSION_STATUS_NOT_STARTED)], { status: "in-progress", orchestrator: ORCH }), MISSING_PATH);
        assert.deepStrictEqual(out.sessions[0].orchestrator, ORCH);
    });
    test("v3 top-level status canonicalized", () => {
        const out = (0, progress_1.normalizeToV4Shape)(v3State([sess(1, progress_1.SESSION_STATUS_COMPLETE)], { status: "completed" }), MISSING_PATH);
    ...
    test("v3 preserves preCancelStatus passthrough", () => {
        const out = (0, progress_1.normalizeToV4Shape)(v3State([sess(1, progress_1.SESSION_STATUS_NOT_STARTED)], {
            status: "cancelled",
    ...
    test("v3 preserves forceClosed passthrough", () => {
        const out = (0, progress_1.normalizeToV4Shape)(v3State([sess(1, progress_1.SESSION_STATUS_COMPLETE)], {
            status: "complete",
            forceClosed: true,
        }), MISSING_PATH);
...
suite("normalizeToV4Shape — v4 input", () => {
    test("v4 per-session orchestrator wins", () => {
        const out = (0, progress_1.normalizeToV4Shape)(v4State([
    ...
    test("v4 derives top orchestrator from in-progress session", () => {
        const out = (0, progress_1.normalizeToV4Shape)(v4State([
            sess(1, progress_1.SESSION_STATUS_COMPLETE, { completedAt: "2026-05-26T11:00:00-04:00" }),
            sess(2, progress_1.SESSION_STATUS_IN_PROGRESS, {
                orchestrator: ORCH,
    ...
    test("v4 derives top verdict from last completed session", () => {
        const out = (0, progress_1.normalizeToV4Shape)(v4State([
            sess(1, progress_1.SESSION_STATUS_COMPLETE, { verificationVerdict: "VERIFIED" }),
            sess(2, progress_1.SESSION_STATUS_COMPLETE, { verificationVerdict: "ISSUES_FOUND" }),
        ], { status: "complete" }), MISSING_PATH);
    ...
    test("v4 ignores stale top-level orchestrator when per-session is present", () => {
        const out = (0, progress_1.normalizeToV4Shape)(v4State([sess(1, progress_1.SESSION_STATUS_IN_PROGRESS, { orchestrator: { engine: "codex", provider: "openai" } })], { orchestrator: ORCH }), MISSING_PATH);
...
suite("normalizeToV4Shape — v2 input", () => {
    test("v2 synthesizes then enriches to v4", () => {
        ...
        const out = (0, progress_1.normalizeToV4Shape)(v2, MISSING_PATH);
        assert.strictEqual(out.schemaVersion, progress_1.SCHEMA_VERSION_V4);
...
suite("normalizeToV4Shape — errors and edges", () => {
    test("null input throws TypeError", () => {
        assert.throws(() => (0, progress_1.normalizeToV4Shape)(null, MISSING_PATH), TypeError);
    });
    test("sessions not an array raises invariant error", () => {
        ...
        assert.throws(() => (0, progress_1.normalizeToV4Shape)(state, MISSING_PATH), progress_1.SessionStateInvariantError);
    });
    test("input is not mutated (pure function)", () => {
        const state = v3State([sess(1, progress_1.SESSION_STATUS_IN_PROGRESS)], {
            orchestrator: ORCH,
            status: "in-progress",
        });
        const originalSessions = state.sessions;
        const originalSession0 = { ...state.sessions[0] };
        const out = (0, progress_1.normalizeToV4Shape)(state, MISSING_PATH);
...
suite("normalizeToV4Shape — readProgress routing", () => {
    test("v3 file still reads through readProgress", () => {
        ...
        const view = (0, progress_1.readProgress)(state, MISSING_PATH);
...
suite("normalizeToV4Shape — verifier fix 1: per-session status aliases", () => {
    test("v3 session with completed alias lands in derived completedSessions", () => {
        const out = (0, progress_1.normalizeToV4Shape)(v3State([
            sess(1, "completed"),
            sess(2, progress_1.SESSION_STATUS_IN_PROGRESS),
        ], { status: "in-progress" }), MISSING_PATH);
        assert.strictEqual(out.sessions[0].status, "complete");
        assert.deepStrictEqual(out.completedSessions, [1]);
    });
    ...
    test("v4 session with completed alias canonicalizes", () => {
        const out = (0, progress_1.normalizeToV4Shape)(v4State([sess(1, "completed", { completedAt: "2026-05-26T11:00:00-04:00" })], { status: "complete" }), MISSING_PATH);
        assert.strictEqual(out.sessions[0].status, "complete");
        assert.deepStrictEqual(out.completedSessions, [1]);
        assert.strictEqual(out.completedAt, "2026-05-26T11:00:00-04:00");
    });
...
suite("normalizeToV4Shape — verifier fix 2: startedAt promotion/derivation", () => {
    ...
    test("v3 all-complete promotes startedAt to last completed", () => {
        const out = (0, progress_1.normalizeToV4Shape)(v3State([sess(1, progress_1.SESSION_STATUS_COMPLETE), sess(2, progress_1.SESSION_STATUS_COMPLETE)], {
            status: "complete",
...
suite("normalizeToV4Shape — idempotence", () => {
    test("idempotent on v3 input", () => {
        ...
        const once = (0, progress_1.normalizeToV4Shape)(state, MISSING_PATH);
        const twice = (0, progress_1.normalizeToV4Shape)(once, MISSING_PATH);
    ...
    test("idempotent preserves passthrough fields", () => {
        const state = v3State([sess(1, progress_1.SESSION_STATUS_NOT_STARTED)], {
            status: "cancelled",
            preCancelStatus: "in-progress",
            forceClosed: false,
        });
...
//# sourceMappingURL=normalizeV4Shape.test.js.map