...
const inProgressSetsService_1 = require("../../providers/inProgressSetsService");
...
suite("extractRecommendation", () => {
    test("parses a well-formed `## Session N: title` + ### Recommended orchestrator block", () => {
        ...
        const rec = (0, inProgressSetsService_1.extractRecommendation)(text, 4, "029-orchestrator");
    ...
    test("supports `## Session N of M: title` heading form", () => {
        ...
        const rec = (0, inProgressSetsService_1.extractRecommendation)(text, 3, "set");
        assert.ok(rec);
    ...
    test("returns null when the session heading is absent", () => {
        ...
        assert.strictEqual((0, inProgressSetsService_1.extractRecommendation)(text, 4, "set"), null);
    });
    ...
    test("does not bleed into the next session's recommendation block", () => {
        ...
        assert.strictEqual((0, inProgressSetsService_1.extractRecommendation)(text, 4, "set"), null, "session 4 has no rec; must NOT pick up session 5's rec");
    });
    test("trims trailing punctuation off the model name", () => {
        ...
        const rec = (0, inProgressSetsService_1.extractRecommendation)(text, 1, "set");
        assert.ok(rec);
        assert.strictEqual(rec.modelName, "Opus 4.7");
    });
});
...
function fakeSet(name, state, startedAt) {
    return {
        name,
        dir: `/tmp/${name}`,
...
suite("listInProgressSets", () => {
    test("returns only in-progress sets, sorted by startedAt asc", () => {
        ...
        const out = (0, inProgressSetsService_1.listInProgressSets)(sets);
        assert.deepStrictEqual(out.map((s) => s.name), ["033-oldest-in-progress", "032-newer-in-progress"], "oldest-checked-out set ranks first");
    });
    test("returns empty array when no sets are in progress", () => {
        ...
        assert.deepStrictEqual((0, inProgressSetsService_1.listInProgressSets)(sets), []);
    });
...
//# sourceMappingURL=inProgressSetsService.test.js.map