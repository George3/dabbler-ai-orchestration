...
const annotationScanner_1 = require("../../commands/annotationScanner");
...
suite("scanAnnotationsForActiveSet — scanFilesForAnnotations", () => {
    test("returns empty list when no annotations are present", () => {
        const root = "/ws";
        const reader = (_p) => "console.log('hello');\n";
        const result = (0, annotationScanner_1.scanFilesForAnnotations)(["/ws/a.ts", "/ws/b.py"], root, now, reader);
    ...
    test("finds one annotation across multiple files", () => {
        ...
        const reader = (p) => files[p];
        const result = (0, annotationScanner_1.scanFilesForAnnotations)(Object.keys(files), root, now, reader);
    ...
    test("file that throws on read is skipped silently", () => {
        ...
        const result = (0, annotationScanner_1.scanFilesForAnnotations)(["/ws/bad.py", "/ws/good.py"], root, now, reader);
        assert.strictEqual(result.length, 1);
    ...
    test("multiple annotations in one file are returned in order", () => {
        ...
        const result = (0, annotationScanner_1.scanFilesForAnnotations)(["/ws/a.py"], root, now, reader);
        assert.strictEqual(result.length, 2);
        assert.deepStrictEqual(result.map((a) => [a.reason, a.line]), [["first", 2], ["second", 4]]);
    });
});
suite("scanAnnotationsForActiveSet — loadHonorAnnotationsToggle", () => {
    test("defaults to true when local-overrides.yaml is missing", () => {
        const readYaml = (_p) => null;
        assert.strictEqual((0, annotationScanner_1.loadHonorAnnotationsToggle)("/ws", readYaml), true);
    });
    test("defaults to true when decision_review key is absent", () => {
        const readYaml = (_p) => ({ other_section: { foo: "bar" } });
        assert.strictEqual((0, annotationScanner_1.loadHonorAnnotationsToggle)("/ws", readYaml), true);
    });
    test("defaults to true when honor_annotations is absent under decision_review", () => {
        const readYaml = (_p) => ({ decision_review: { other: "thing" } });
        assert.strictEqual((0, annotationScanner_1.loadHonorAnnotationsToggle)("/ws", readYaml), true);
    });
    test("returns false when explicitly set to false", () => {
        const readYaml = (_p) => ({ decision_review: { honor_annotations: false } });
        assert.strictEqual((0, annotationScanner_1.loadHonorAnnotationsToggle)("/ws", readYaml), false);
    });
    test("returns true when explicitly set to true", () => {
        const readYaml = (_p) => ({ decision_review: { honor_annotations: true } });
        assert.strictEqual((0, annotationScanner_1.loadHonorAnnotationsToggle)("/ws", readYaml), true);
    });
    test("non-boolean value falls back to default true", () => {
        const readYaml = (_p) => ({ decision_review: { honor_annotations: "yes" } });
        assert.strictEqual((0, annotationScanner_1.loadHonorAnnotationsToggle)("/ws", readYaml), true);
    });
    test("reads from ai_router/local-overrides.yaml under the workspace root", () => {
        ...
        (0, annotationScanner_1.loadHonorAnnotationsToggle)("/ws", readYaml);
...
suite("scanAnnotationsForActiveSet — loadExistingQueueEntries", () => {
    ...
    test("returns dedup keys for annotation entries; skips command entries (file=null)", () => {
        const dir = makeTmpDir();
        try {
            (0, decisionReviewQueue_1.appendQueueEntry)(dir, {
    ...
    test("uses the injected reader (test seam)", () => {
        ...
        const dir = makeTmpDir();
        try {
            fs.writeFileSync(path.join(dir, decisionReviewQueue_1.QUEUE_FILENAME), "");
            const result = (0, annotationScanner_1.loadExistingQueueEntries)(dir, reader);
...
suite("scanAnnotationsForActiveSet — scan globs", () => {
    test("SCAN_GLOB includes common source extensions", () => {
        for (const ext of ["ts", "py", "go", "rs", "java", "cs", "sh"]) {
            assert.ok(annotationScanner_1.SCAN_GLOB.includes(ext), `${ext} missing from SCAN_GLOB`);
        }
    });
    test("SCAN_EXCLUDE_GLOB filters obvious noise dirs", () => {
        for (const seg of ["node_modules", "dist", ".venv", ".git", "__pycache__"]) {
            assert.ok(annotationScanner_1.SCAN_EXCLUDE_GLOB.includes(seg), `${seg} missing from SCAN_EXCLUDE_GLOB`);
        }
    });
});
suite("scanAnnotationsForActiveSet — end-to-end dedup flow", () => {
    test("re-scanning an unchanged file produces no new appends", () => {
        ...
        try {
            const root = "/ws";
            const reader = (_p) => '# @dabbler:outsource-review("test")\n';
            const round1 = (0, annotationScanner_1.scanFilesForAnnotations)(["/ws/x.py"], root, now, reader);
            ...
            const round2 = (0, annotationScanner_1.scanFilesForAnnotations)(["/ws/x.py"], root, now, reader);
            const existing = (0, annotationScanner_1.loadExistingQueueEntries)(dir);
    ...
    test("a newly-added annotation in an unchanged file IS appended on next scan", () => {
        ...
        try {
            ...
            const reader = (_p) => text;
            const round1 = (0, annotationScanner_1.scanFilesForAnnotations)(["/ws/x.py"], root, now, reader);
            for (const ann of round1)
                (0, decisionReviewQueue_1.appendQueueEntry)(dir, ann);
            assert.strictEqual((0, annotationScanner_1.loadExistingQueueEntries)(dir).length, 1);
            ...
            const fresh = deduplicateAnnotations(round2, (0, annotationScanner_1.loadExistingQueueEntries)(dir));
...
//# sourceMappingURL=scanAnnotationsForActiveSet.test.js.map