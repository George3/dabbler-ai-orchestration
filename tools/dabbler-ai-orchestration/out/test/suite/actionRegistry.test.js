...
suite("ActionRegistry", () => {
    test("ROW_ACTIONS exposes the 14 menu-surface actions (Set 048 S3 reshape + Set 049 S1 hygiene + Set 049 S4 rip-out)", () => {
        ...
        assert.strictEqual(ActionRegistry_1.ROW_ACTIONS.length, 14);
    });
    test("openAiAssignment fully removed (L3)", () => {
        for (const a of ActionRegistry_1.ROW_ACTIONS) {
            assert.notStrictEqual(a.id, "dabblerSessionSets.openAiAssignment", "openAiAssignment must not appear in ROW_ACTIONS — L3 operator-locked removal");
        }
    });
    test("category discriminator partitions actions for the two-step QuickPick", () => {
        const cats = new Set(ActionRegistry_1.ROW_ACTIONS.map((a) => a.category));
        assert.deepStrictEqual(cats, new Set(["openFile", "copyEval", "flat"]), "every action must declare one of the three menu categories");
    });
    test("Open File submenu locked to exactly four entries (L2)", () => {
        const openFile = ActionRegistry_1.ROW_ACTIONS.filter((a) => a.category === "openFile").map((a) => a.id);
        assert.deepStrictEqual(openFile, [
            "dabblerSessionSets.openSpec",
            "dabblerSessionSets.openActivityLog",
            "dabblerSessionSets.openChangeLog",
            "dabblerSessionSets.openSessionState",
    ...
    test("copyEval submenu — spec-review is always enabled", () => {
        for (const st of ["in-progress", "not-started", "complete", "cancelled"]) {
            assert.ok(ids(fakeSet(st), ALL_SUPPORTED).includes("dabbler.copySpecReviewPrompt"), `copySpecReviewPrompt missing for state=${st}`);
        }
    });
    test("copyEval submenu — session-accomplishments gated on sessionsCompleted > 0", () => {
        ...
        assert.ok(!ids(zero, ALL_SUPPORTED).includes("dabbler.copySessionAccomplishmentsPrompt"), "session-accomplishments should NOT surface before any session completes");
        assert.ok(ids(one, ALL_SUPPORTED).includes("dabbler.copySessionAccomplishmentsPrompt"), "session-accomplishments should surface once one session is closed");
        assert.ok(ids(three, ALL_SUPPORTED).includes("dabbler.copySessionAccomplishmentsPrompt"), "session-accomplishments should surface on a completed set too");
    });
    test("copyEval submenu — set-accomplishments gated on state === complete", () => {
        for (const st of ["in-progress", "not-started", "cancelled"]) {
            assert.ok(!ids(fakeSet(st, { sessionsCompleted: 5 }), ALL_SUPPORTED)
                .includes("dabbler.copySetAccomplishmentsPrompt"), `set-accomplishments leaked onto state=${st}`);
        }
        assert.ok(ids(fakeSet("complete"), ALL_SUPPORTED).includes("dabbler.copySetAccomplishmentsPrompt"), "set-accomplishments should surface only on complete state");
    });
    test("copyEval submenu — start-next-session gated on non-terminal rows (L5)", () => {
        for (const st of ["in-progress", "not-started"]) {
            assert.ok(ids(fakeSet(st), ALL_SUPPORTED).includes("dabbler.copyStartNextSessionPrompt"), `start-next-session missing for state=${st}`);
        }
        for (const st of ["complete", "cancelled"]) {
            assert.ok(!ids(fakeSet(st), ALL_SUPPORTED).includes("dabbler.copyStartNextSessionPrompt"), `start-next-session leaked onto terminal state=${st}`);
        }
    });
    test("copyEval submenu — start-next-parallel-session gated on non-terminal rows (Set 049 S1)", () => {
        for (const st of ["in-progress", "not-started"]) {
            assert.ok(ids(fakeSet(st), ALL_SUPPORTED).includes("dabbler.copyStartNextParallelSessionPrompt"), `start-next-parallel-session missing for state=${st}`);
        }
        for (const st of ["complete", "cancelled"]) {
            assert.ok(!ids(fakeSet(st), ALL_SUPPORTED).includes("dabbler.copyStartNextParallelSessionPrompt"), `start-next-parallel-session leaked onto terminal state=${st}`);
        }
    });
    test("checkOutOrchestrator fully retired (Set 049 S4 rip-out)", () => {
        for (const st of ["in-progress", "not-started", "complete", "cancelled"]) {
            assert.ok(!ids(fakeSet(st), ALL_SUPPORTED).includes("dabbler.checkOutOrchestrator"), `checkOutOrchestrator must not appear in any state (retired Set 049 S4); leaked onto state=${st}`);
        }
        for (const a of ActionRegistry_1.ROW_ACTIONS) {
            assert.notStrictEqual(a.id, "dabbler.checkOutOrchestrator", "dabbler.checkOutOrchestrator must not appear in ROW_ACTIONS — Set 049 S4 rip-out");
        }
    });
    test("openOrchestratorWriterLog stays for all states (writer log preserved per Set 049 T5)", () => {
        for (const st of ["in-progress", "not-started", "complete", "cancelled"]) {
            assert.ok(ids(fakeSet(st), ALL_SUPPORTED).includes("dabbler.openOrchestratorWriterLog"), `openOrchestratorWriterLog missing for state=${st}`);
        }
    });
    ...
    test("migrate (v3) appears only when needsMigration + target=3 (v1/v2 or broken-v3)", () => {
        ...
        assert.ok(!ids(needsV3, ALL_SUPPORTED).includes("dabblerSessionSets.migrateToV4"), "v3-target row should not surface the v4 migrator");
        assert.ok(!ids(needsV4, ALL_SUPPORTED).includes("dabblerSessionSets.migrate"), "v4-target row should not surface the v3 migrator");
    ...
    test("categorizedActions partitions and pre-sorts within each category", () => {
        const cats = (0, ActionRegistry_1.categorizedActions)(fakeSet("in-progress", { sessionsCompleted: 2 }), ALL_SUPPORTED);
    ...
    test("categorizedActions on a complete set surfaces set-accomplishments and NOT start-next", () => {
        const cats = (0, ActionRegistry_1.categorizedActions)(fakeSet("complete", { sessionsCompleted: 5 }), ALL_SUPPORTED);
        const ids = cats.copyEval.map((a) => a.id);
        assert.ok(ids.includes("dabbler.copySetAccomplishmentsPrompt"));
        assert.ok(!ids.includes("dabbler.copyStartNextSessionPrompt"), "start-next-session must not surface on complete (terminal) state");
        assert.ok(!ids.includes("dabbler.copyStartNextParallelSessionPrompt"), "start-next-parallel-session must not surface on complete (terminal) state");
    });
});
