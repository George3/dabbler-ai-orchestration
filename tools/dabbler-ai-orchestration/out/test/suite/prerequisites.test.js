...
function writeState(setDir, status, totalSessions = 2) {
    const sessions = [];
...
function specWithPrereqs(totalSessions, prereqs) {
    return [
        "# Test Set",
        "",
...
suite("Set 047 / S5 — parsePrerequisites", () => {
    test("returns null when field is absent", () => {
        ...
        try {
            ...
            const result = (0, fileSystem_1.parsePrerequisites)(path.join(set, "spec.md"));
...
suite("Set 047 / S5 — readSessionSets cross-reference derivation", () => {
    ...
    test("blockedByPrereqs=true when prereq target is in-progress", () => {
        ...
        try {
            ...
            writeSpec(setDir(root, "047-blocked"), specWithPrereqs(2, [
                "prerequisites:",
                "  - slug: 046-prereq",
                "    condition: complete",
            ].join("\n")));
            writeState(setDir(root, "047-blocked"), "not-started", 2);
            const sets = (0, fileSystem_1.readSessionSets)(root);
    ...
    test("blockedByPrereqs=false when prereq target is complete", () => {
        ...
        try {
            ...
            writeSpec(setDir(root, "047-unblocked"), specWithPrereqs(1, [
                "prerequisites:",
                "  - slug: 046-done",
                "    condition: complete",
            ].join("\n")));
    ...
    test("blockedByPrereqs=true when prereq slug doesn't resolve (typo / missing set)", () => {
        const root = setupRepo();
        try {
            writeSpec(setDir(root, "047-typo"), specWithPrereqs(1, [
                "prerequisites:",
    ...
    test("blockedByPrereqs respects multiple prereqs (ANY unsatisfied → blocked)", () => {
        ...
        try {
            ...
            writeSpec(setDir(root, "047-multi"), specWithPrereqs(1, [
                "prerequisites:",
                "  - slug: 044-done",
                "    condition: complete",
...
suite("Set 047 / S5 — blockedByPrereqsBadge", () => {
    ...
    test("renders the badge when blockedByPrereqs=true on a non-terminal row", () => {
        assert.strictEqual((0, SessionSetsModel_1.blockedByPrereqsBadge)(fakeSet({ state: "not-started", blockedByPrereqs: true })), "[BLOCKED BY PREREQS]");
        assert.strictEqual((0, SessionSetsModel_1.blockedByPrereqsBadge)(fakeSet({ state: "in-progress", blockedByPrereqs: true })), "[BLOCKED BY PREREQS]");
    });
    test("suppresses the badge on complete/cancelled rows (no longer actionable)", () => {
        assert.strictEqual((0, SessionSetsModel_1.blockedByPrereqsBadge)(fakeSet({ state: "complete", blockedByPrereqs: true })), "");
        assert.strictEqual((0, SessionSetsModel_1.blockedByPrereqsBadge)(fakeSet({ state: "cancelled", blockedByPrereqs: true })), "");
    });
    test("renders nothing when blockedByPrereqs=false", () => {
        assert.strictEqual((0, SessionSetsModel_1.blockedByPrereqsBadge)(fakeSet({ blockedByPrereqs: false })), "");
    });
});
//# sourceMappingURL=prerequisites.test.js.map