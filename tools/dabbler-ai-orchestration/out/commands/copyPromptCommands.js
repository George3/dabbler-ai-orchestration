...
exports.__forTests = void 0;
exports.buildSpecReviewPrompt = buildSpecReviewPrompt;
exports.buildSessionAccomplishmentsPrompt = buildSessionAccomplishmentsPrompt;
exports.buildSetAccomplishmentsPrompt = buildSetAccomplishmentsPrompt;
exports.sanitizeSlugForPrompt = sanitizeSlugForPrompt;
exports.buildStartNextSessionPrompt = buildStartNextSessionPrompt;
exports.buildStartNextParallelSessionPrompt = buildStartNextParallelSessionPrompt;
exports.registerCopyPromptCommands = registerCopyPromptCommands;
...
function registerCopyPromptCommands(context) {
    context.subscriptions.push(vscode.commands.registerCommand("dabbler.copySpecReviewPrompt", async (item) => {
        if (!item?.set)
            return;
        const prompt = buildSpecReviewPrompt(item.set);
        await copyToClipboard(prompt, `Copied: Spec-review prompt for ${item.set.name}`);
    }), vscode.commands.registerCommand("dabbler.copySessionAccomplishmentsPrompt", async (item) => {
        if (!item?.set)
            return;
    ...
    }), vscode.commands.registerCommand("dabbler.copySetAccomplishmentsPrompt", async (item) => {
        if (!item?.set)
            return;
    ...
    }), vscode.commands.registerCommand("dabbler.copyStartNextSessionPrompt", async (item) => {
        if (!item?.set)
            return;
    ...
    }), vscode.commands.registerCommand("dabbler.copyStartNextParallelSessionPrompt", async (item) => {
        if (!item?.set)
            return;
...
//# sourceMappingURL=copyPromptCommands.js.map