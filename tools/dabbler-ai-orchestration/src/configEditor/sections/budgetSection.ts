...
import { asNumber, escapeHtml, fieldSource, getByPath, indicatorHtml } from "./helpers";
...
export function render(state: SectionState): SectionRenderResult {
  const sharedThreshold = asNumber(getByPath(state.budget, "threshold_usd"));
  const localThreshold = asNumber(getByPath(state.localOverrides, "threshold_usd"));
  ...
  const sharedWarn = asNumber(getByPath(state.budget, "warn_at_percent"));
  const localWarn = asNumber(getByPath(state.localOverrides, "warn_at_percent"));
