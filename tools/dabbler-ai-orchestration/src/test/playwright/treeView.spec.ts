...
// Shared per-test container of disposables so cleanup runs even on
// assertion failure.
interface PerTest {
  tmpPath?: string;
...
// ---------------------------------------------------------------------
test("renders three not-started sets under Not Started", async () => {
  const per: PerTest = {};
  try {
    per.tmpPath = makeTmpDir("dabbler-pw-fresh");
    const a = makeSet(per.tmpPath, "scenario-fresh-a", 3);
    // Sibling sets in the same repo via the harness shim.
    makeAdditionalSet(a, "scenario-fresh-b", 4);
    makeAdditionalSet(a, "scenario-fresh-c", 3);
    ...
    expect(joined).toMatch(/Not Started\s+\(3\)/);
    expect(joined).toContain("scenario-fresh-a");
    expect(joined).toContain("scenario-fresh-b");
...
// ---------------------------------------------------------------------
test("renders an in-flight set with the 'in flight' annotation", async () => {
  const per: PerTest = {};
  try {
    per.tmpPath = makeTmpDir("dabbler-pw-inflight");
    const h = makeSet(per.tmpPath, "scenario-inflight", 3);
    // Complete session 1 cleanly, then start session 2 and leave it
    // in flight.
    startSession(h, 1);
    makeActivity(h, 1);
    makeDisposition(h, 1, /* isFinal */ false);
    const r1 = closeSession(h, 1);
    if (r1.exit !== 0) throw new Error(`close 1 failed: ${r1.stderr}`);
    startSession(h, 2);
...
test("renders a fully closed set under Done with N/N progress", async () => {
  ...
  try {
    ...
    const h = makeSet(per.tmpPath, "scenario-done", 3);
    driveHappyPath(h, 3);
...
// ---------------------------------------------------------------------
test("renders a cancelled set under the Cancelled bucket", async () => {
  const per: PerTest = {};
  try {
    per.tmpPath = makeTmpDir("dabbler-pw-cancel");
    const h = makeSet(per.tmpPath, "scenario-cancel", 4);
    ...
    expect(joined).toMatch(/Cancelled\s+\(1\)/);
...
// ---------------------------------------------------------------------
test("renders [FORCED] badge on a force-closed set", async () => {
  const per: PerTest = {};
  try {
    per.tmpPath = makeTmpDir("dabbler-pw-forced");
    const h = makeSet(per.tmpPath, "scenario-forced", 3);
    // Session 1 healthy.
    startSession(h, 1);
    makeActivity(h, 1);
    makeDisposition(h, 1, false);
    ...
    startSession(h, 2);
    const r2 = closeSession(h, 2, { force: true });
    if (r2.exit !== 0) throw new Error(`force-close 2 failed: ${r2.stderr}`);
