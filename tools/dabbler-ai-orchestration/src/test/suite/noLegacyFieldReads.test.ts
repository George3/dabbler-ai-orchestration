...
const ACCESS_PATTERNS = [
  ...LEGACY_FIELDS.map(
    (f) => new RegExp(`\\b${RAW_STATE_VARS}\\.${f}\\b`),
  ),
  ...LEGACY_FIELDS.map(
    (f) => new RegExp(`\\b${RAW_STATE_VARS}\\[["']${f}["']\\]`),
  ),
];
...
function isNoqaLine(line: string): boolean {
...
suite("D13 lint — no legacy field reads in application code", () => {
  test("no direct access to currentSession / totalSessions / completedSessions", () => {
    ...
    if (violations.length > 0) {
      const formatted = violations
        ...
        .join("\n");
      assert.fail(
        `D13 lint violation: ${violations.length} direct legacy-field ` +
  ...
  test("lint scanner finds the source tree (sanity check)", () => {
    ...
    assert.ok(
      text.includes("noqa: D13"),
      "fileSystem.ts should carry the D13 carve-out marker " +
