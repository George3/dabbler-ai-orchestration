...
| `verification_verdict` | string or omitted | conditional | **Required on the `api` verification path.** The verifier's pass/fail outcome: `"VERIFIED"` or `"ISSUES_FOUND"`. Set to the value returned by `parse_verification_response()` in Step 6 of the workflow before authoring the rest of the disposition. `close_session` reads this field via `resolve_close_verdict()` and writes it to `session-state.json`'s per-session `verificationVerdict`. Omit (not `null`) when verification was manual / skipped / --no-router — `close_session` then records `verificationVerdict: null` in the state file. Non-canonical extension tokens are accepted but trigger a stderr warning. |
