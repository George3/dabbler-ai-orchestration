...
- **Version-bump correctness.** If the set ships a Marketplace or
  PyPI release, the version walk in `CLAUDE.md` and the change-log
  entries should both name the new version, and the tag-driven
  GitHub Actions workflow should be quoted (per
  `reference_publish_via_github_actions`).
