# GPT-5.4 manual-paste prompt — custom-tree pivot audit
...
>
> **Why manual paste:** the GPT-5.4 API is currently 429-rate-limited
> from Set 029 Session 2's verification rounds. Manual paste in
> GitHub Copilot bypasses the API throttle. Same model, same depth.
...
**Important context:** earlier this same day, you may have reviewed
a different proposal in the same codebase — a "per-workspace
orchestrator markers" design at
`docs/proposals/2026-05-18-per-workspace-orchestrator-markers/proposal.md`.
The proposal below **supersedes that one**. The per-workspace
approach solved a real bug (cross-window marker contamination) but
the operator and I concluded its scope is wrong — the underlying
issue is the identity model, not just the marker path. The proposal
below changes the identity model: orchestrator state becomes
per-session-set, not per-workspace. Please review THIS proposal on
its own merits; do not assume the per-workspace decisions carry over.

Review the proposal below and give your verdict on each of the ten
open questions (Q1–Q10) plus an overall recommendation. Structured
...
**Supersedes:** `../2026-05-18-per-workspace-orchestrator-markers/proposal.md`
...
1. **Cross-window contamination.** The operator's workflow runs
   ...
   `dabbler-access-harvester`). Every SessionStart hook in every
   window writes the same `~/.dabbler/current-orchestrator.json`.
   The most-recently-started session wins; the others' gauges
   silently display wrong data ("the gauge actively lies about
   what model each window is running"). The per-workspace audit
   proposed hashing the workspace root path into a per-workspace
   marker filename. That works but only addresses one axis of the
   underlying identity problem.
...
v0.14.2 is committed but NOT published to Marketplace. The pivot
ships as a clean cutover under 0.14.3 (or 0.15.0 if reviewers
prefer a minor bump for the architectural change). No external
...
- Layer 3: completely rewritten. The Playwright `_electron.launch`
  path still works; the assertions all need updating because the
  rendered DOM is now webview-generated, not native-tree.
...
non-Claude work risks an under-cooked v1 of either.
...
~50 LOC of `package.json` changes.
...
Where you have a strong divergent view from the proposal — or where
you suspect the operator and I have overlooked a structural risk
— say so plainly. The operator wants your real read, not polite
ratification.
