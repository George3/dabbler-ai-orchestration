...
|---|---|---|---|
| `dabbler-ai-router` | `0.18.0` | **Version-bumped 2026-06-12, publish PENDING** — metrics `session_set` write-boundary normalization. Tag `v0.18.0` pushed with operator authorization; run id recorded post-publish. Last published: `0.17.0` (2026-06-12, tag `v0.17.0`, release.yml run 27417089861). | `pyproject.toml`, `ai_router/CHANGELOG.md` |
| `dabbler-ai-orchestration` VS Code extension | `0.31.0` | **Version-bumped 2026-06-12, publish PENDING** — Cost Dashboard session-set normalization + README two-tier cost/attention framing. Tag `vsix-v0.31.0` pushed with operator authorization; first release through the green-Test gate; run id recorded post-publish. Last published: `0.30.0` (2026-06-12, tag `vsix-v0.30.0`, publish-vscode.yml run 27417089796). | `tools/dabbler-ai-orchestration/package.json`, `tools/dabbler-ai-orchestration/CHANGELOG.md` |

### Recent version walk

- **Extension `0.31.0` + `dabbler-ai-router 0.18.0`** — ad-hoc curator pass (post-Set-062): Cost Dashboard `session_set` normalization (one slug-keyed row per set; no machine paths in the CSV export), README two-tier cost/attention framing, Layer-3 Playwright suite repair (first-ever green `Test` run), and the green-Test release prerequisite. Publishes pending; run ids recorded post-publish.
