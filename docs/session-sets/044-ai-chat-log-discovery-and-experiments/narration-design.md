...
**Channel resolved (S3, 2026-05-22):** Copilot 1.0.51's
custom-instructions surface is **`AGENTS.md` files**, loaded
from the git root and the current working directory by default
(per `copilot help environment`), with additional search
directories configurable via the
`COPILOT_CUSTOM_INSTRUCTIONS_DIRS` env var. The
`--no-custom-instructions` CLI flag disables loading
("Disable loading of custom instructions from AGENTS.md and
related files"). A second surface,
`.github/copilot-instructions.md`, is generated by
`copilot init`.

**v1 choice for the synthetic-set runs:** `AGENTS.md` at the
synthetic-set workspace root (`c:\tmp\dabbler-log-harvest\
synthetic-set\AGENTS.md`). Reason: simplest possible
delivery — no `copilot init` invocation needed, no `.github/`
sub-directory required, single file the operator can edit
between baseline and narrated runs to flip the treatment.

**Channel-length status (per gemini-pro consensus
2026-05-22):** the `AGENTS.md` channel has no documented
length cap. The full instruction text below is approximately
1100 characters when concrete values are substituted; comfortably
under any plausible limit. The compact form in §7.4 remains
documented for resilience but is not needed for the v1 channel.
...
- [x] **Copilot channel TBDs resolved** — §7.1 channel is
      `AGENTS.md` at synthetic-set workspace root, per
...
1. ~~(LOCK GATE) What is the Copilot 1.0.51 system-prompt /
   custom-instructions delivery channel?~~ **Resolved S3
   2026-05-22:** `AGENTS.md` at the synthetic-set workspace
   root (see §7.1 for full enumeration of candidate surfaces).
