...
+            merged = sorted(set(existing_list) | set(from_events))
+            if merged and merged != existing_list:
                 try:
                     state_path = os.path.join(
                         session_set_dir, "session-state.json"
@@ -931,22 +945,50 @@ def _run_repair(
...
-                        "session-state.json"
-                    )
+                    # Distinguish "added missing entries to a partial
+                    # snapshot" from "wrote the array from scratch on
+                    # a snapshot that had no array at all." Both are
+                    # legitimate apply outcomes; the message line
+                    # helps the operator tell at a glance.
+                    if existing_list:
+                        messages.append(
+                            "repair applied: merged "
+                            f"completedSessions={merged} into "
+                            "session-state.json (union of snapshot "
+                            f"{existing_list} and events "
+                            f"{from_events})"
+                        )
+                    else:
+                        messages.append(
...
+            elif merged and merged == existing_list and from_events:
+                # Snapshot's array already covers everything the
+                # ledger reconstruction would add. The synthetic
+                # closeout events we appended above brought the
+                # ledger up; the snapshot was already correct.
+                # Surface this as a distinct outcome so an operator
+                # who hand-migrated a set sees that their array was
+                # preserved verbatim.
+                messages.append(
+                    "repair preserved completedSessions="
+                    f"{existing_list} in session-state.json "
...
index 5c30ee5..d3976ce 100644
--- a/ai_router/docs/close-out.md
+++ b/ai_router/docs/close-out.md
@@ -499,13 +499,32 @@ The drift shapes the walk detects:
...
+   `completedSessions[]` and (b) the distinct
+   `closeout_succeeded` session numbers in the now-repaired ledger.
+   The union is **monotone-up only** — repair appends session
+   numbers to bring the snapshot up to ledger reality but never
+   removes a session number the operator hand-authored. Two
+   apply outcomes are distinguished in the `messages` line:
+     - *Backfilled* (snapshot's array was empty or absent): the
+       repair writes the events-ledger reconstruction in full —
+       `repair applied: backfilled completedSessions=[...]`.
+     - *Merged* (snapshot's array exists but differs from the
+       ledger view): the repair writes the union and reports both
+       sources — `repair applied: merged completedSessions=[...]
+       (union of snapshot [...] and events [...])`.
+     - *Preserved* (snapshot's array is already a superset of the
+       ledger view): no snapshot rewrite happens; the message line
+       reports `repair preserved completedSessions=[...] (snapshot
+       already a superset of the events-ledger reconstruction)` so
+       the operator sees the no-op explicitly.
...
index 1c9a467..cc03e46 100644
--- a/ai_router/tests/test_close_session_session4.py
+++ b/ai_router/tests/test_close_session_session4.py
@@ -612,6 +612,147 @@ def test_repair_detects_mixed_mode_drift(closeable_set: Path):
