...
\* **REVISED per audit Q1/S2 (2026-05-17):** Claude Code has no
per-message effort slider; treating the most recent `/think*`
invocation as "current effort" would recreate the false-confidence
failure mode this feature is designed to prevent (both reviewers
agreed). Locked design: effort gauge shows **Medium (default)** for
Claude sessions. If a `/think*` invocation is observed during the
session, the gauge displays the corresponding tier with
`signalKind: "last-observed"` and an "(last /think)" sublabel
qualifier, rendered with hollow-rim visual treatment to distinguish
from a live signal. Resets to Medium on `SessionStart`.
...
- **Q1 — Claude Code effort representation.** Locked: Medium default;
  ...
  with "(last /think)" sublabel suffix. Reset to Medium on
  `SessionStart`. → `audit-summary.md` §Q1.
...
- **S5**: Windows atomic-write contention with file watcher →
  retry loop (3 attempts, 50/150/400ms backoff) in all marker
  writers (Session 2 / Session 3 implementation).
...
1. Author the design proposal as a single markdown doc at
   ...
   "Goal state" section).
2. Author `route_audit.py` that calls `ai_router.route(...)` with
   `task_type='cross-provider-audit'`, routing to **two** verifiers
   (gpt-5-4 and gemini-pro) per the audit-then-spec pattern. Dump
   `RouteResult` to JSON before any attribute access (per memory
...
6. Verify Session 1 itself via a single `task_type='session-verification'`
   call (gpt-5-4) before close-out.

**Creates:**
- `docs/proposals/2026-05-17-model-effort-gauges-design-audit/proposal.md`
- `docs/proposals/2026-05-17-model-effort-gauges-design-audit/route_audit.py`
...
- `docs/session-sets/029-orchestrator-model-effort-gauges/session-reviews/session-001/`
  (prompt + verify-result + session-001-review.md)
...
2. **Webview provider.** Implement
   ...
   `signalKind` values (per audit-summary §"Visual treatment by signalKind"):
   - `current`: solid fill, full opacity
   - `configured-default`: diagonal stripes fill
   - `last-observed`: hollow rim + filled needle
   - `manual`: solid + small operator-icon overlay
...
3. **Marker-file reader and watcher.** Use `vscode.workspace.createFileSystemWatcher`
   ...
   (>`stalenessMaxSec`, default 28800s = 8h): striped fill at ~60%
   opacity + "last updated Xh ago" annotation, no install-hook CTA.
...
5. **Claude Code SessionStart hook installer.** New command
   ...
   **Marker writer implements retry loop** (3 attempts: 50ms / 150ms / 400ms
   backoff) per audit S5 to handle Windows file-lock contention with
   the VS Code file watcher.
6. **Effort tracking (best-effort).** Also install a `UserPromptSubmit`
   ...
   and `effort.native: "/think"` (or megathink/ultrathink). **If
...
7. **Layer 3 Playwright smoke + screenshot assertions.** Scenarios at
   ...
   - rewrite with `effort.signalKind: "last-observed"`, assert
     hollow-rim visual treatment on effort gauge
   - rewrite `updatedAt` to 9h ago, assert stale state (stripes + "last
     updated 9h ago" annotation)
   - **Screenshot assertion** verifies the view container ordering
     (orchestrator indicator above session sets tree).
8. **Version bump:** `package.json` 0.13.17 → 0.13.18.
9. **CHANGELOG:** new entry under 0.13.18 noting Claude-only v1
   preview with explicit limitations: starting model only (no runtime
   `/model` detection in v1); effort best-effort (Medium default plus
   last-observed `/think*` if `UserPromptSubmit` hook supports message
   text); manual-override quickpick available for any state the hook
   misses. Mark non-Claude paths as "coming in 0.14.0".
...
1. **Codex detection (auto).** Read `~/.codex/config.toml` on extension
   ...
   in the hover tooltip: "configured default from `~/.codex/config.toml` —
   does not track runtime `/model` changes". Marker writer reuses the
   retry-loop helper from Session 2.
...
4. **Manual-override quickpick** (`dabbler.setOrchestrator`):
   - Multi-step quickpick: provider → model → effort → thinking on/off
     → writes marker file with `signalKind: "manual"`, `confidence: "high"`.
   - **MRU ordering**: most recently used `<provider, model, effort, thinking>`
     tuples surface first. Stored in `~/.dabbler/orchestrator-mru.json`.
   ...
   - "(create new hotkey binding)" item at bottom of quickpick: copies
     the `keybindings.json` snippet to clipboard pre-filled with the
     current selection.
...
6. **Playwright smoke expansion.** Add scenarios:
   - `signalKind: "configured-default"` for Codex — verify striped fill
     visual treatment on both gauges
   - `signalKind: "manual"` for Gemini and Copilot — verify operator-icon
...
- **R2 — Hook payload format drift.** Claude Code's Stop hook payload
  schema may change between extension versions. Mitigation: the
  helper script (`write-orchestrator-marker.js`) parses defensively
  and falls back to a "Claude (unknown model)" marker on schema
  miss; no crash.
...
- **R4 — Marker-file race conditions.** Multiple orchestrator
  ...
  atomic writes (write + rename) per the audit's Q1 resolution.
- **R5 — Windows atomic-write contention** (added per audit S5).
  ...
  target. Mitigation: all marker writers (Claude SessionStart hook
  script, Codex config.toml watcher, manual-override quickpick)
  implement retry loop with exponential backoff: 3 attempts at
  50ms / 150ms / 400ms. Helper shared across all four writer paths.
...
- Audit calls (S1): `task_type='cross-provider-audit'`, two
  verifiers (gpt-5-4 + gemini-pro).
- Session-end verification (S1, S2, S3, S4): `task_type='session-verification'`,
  single verifier (gpt-5-4).
- Implementation work (S2, S3, S4): pure Claude tokens, no router
  invocation per memory `feedback_ai_router_usage`.

## Total estimated cost

$0.55 – $1.55 (sum of per-session estimates above; mostly S1's
two-verifier audit). Well within the operator's typical NTE range.
