"""Route Set 029 Session 4 verification — Round A (provider-layer + types).

Round A scope: OrchestratorAccordion + MarkerWatchService +
ActionRegistry + suppressionState + sessionSetsWebviewProtocol +
the Q1-Q7 prompt. Round B covers CustomSessionSetsView + client.js
(the integration surface).

Per memory `feedback_split_large_verification_bundles`: bundle
sized to stay under the 700 LOC ceiling that gpt-5-4 timeouts at.
Pinned to gemini-pro per the S3 escape-hatch pattern.
...
def main() -> int:
    ...
    files = [
        ("src/providers/OrchestratorAccordion.ts", "typescript"),
        ("src/providers/MarkerWatchService.ts", "typescript"),
        ("src/providers/ActionRegistry.ts", "typescript"),
        ("src/providers/suppressionState.ts", "typescript"),
        ("src/types/sessionSetsWebviewProtocol.ts", "typescript"),
    ]
