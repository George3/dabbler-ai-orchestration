<!--
  ...
  docs/templates/consumer-bootstrap/start-here.md.template in the
  dabbler-ai-orchestration repo. Edits here are overwritten on the next
  scaffold and will fail the CI snapshot check. Change the template instead.
-->
# Start here — running sessions in `acme-app`
...
1. **Engine file** — you arrived from `CLAUDE.md` / `AGENTS.md` / `GEMINI.md`,
   which pointed you here.
...
## Step 1 — Resolve THE active session set (one machine-checkable rule)

> **Rule (verbatim — CI enforces "exactly one active set"):**
> The active session set is the single directory `docs/session-sets/<NNN-slug>/`
> whose `session-state.json` has `status: "in-progress"`. **There must be at
> most one.** If none is `in-progress`, the next set to start is the
> `not-started` set with the **lowest** `NNN-` prefix. Sets whose status is
> `"complete"` or `"cancelled"` are skipped.

This is exactly what `ai_router.find_active_session_set()` computes — you can
let the CLI resolve it, or apply the rule by reading each set's
`session-state.json`. Never infer state from file presence; read the `status`
field.

If two sets are `in-progress` at once, that is a drift error, not a choice to
...
.venv/Scripts/python.exe -m ai_router.start_session \
    ...
    --engine <claude-code|codex|gemini|copilot> \
