...
> **Audience:** Codex (OpenAI) and GitHub Copilot agents that look for
> an `AGENTS.md` at the repo root. Claude Code reads `CLAUDE.md`;
> Gemini Code Assistant reads `GEMINI.md`. All three files describe the
> same role and rules — only the agent-specific bootstrap differs.
...
### Codex / GitHub Copilot bootstrap (Windows)
